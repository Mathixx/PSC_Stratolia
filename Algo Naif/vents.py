...
def affiche_vents3D(temps,longmin, longmax, latmin,latmax):
     ...
     ax.quiver(X, Y, Z, u, v, 0, length=1, arrow_length_ratio=0.6, normalize=True, pivot = "middle" )   
...
def affiche_vents2D(temps, longmin, longmax, latmin,latmax, pression, indice = False, ret=False):
    ...
    if ret: 
        ...
        plt.ylabel('Latitude')
        plt.title(f'Vecteurs vents à {pres[closest_pressure_idx]} hPa / Altitude : {format(alts[closest_pressure_idx],'.1f')} km)')
        plt.show()
...
def affiche_ventsGIF(temps, longmin, longmax, latmin,latmax,show = True, save = False):
    def animv(num,qr,temps, longmin, longmax, latmin,latmax):
        ...
        long_indices = np.where((longmin <= longs) & (longs <= longmax))[0]
        lat_indices = np.where((latmin <= lats) & (lats <= latmax))[0]
        u = wind_data['data'][temps, nummod, :, :][np.ix_(lat_indices, long_indices)[0], np.ix_(lat_indices, long_indices)[1], 0]
        v = wind_data['data'][temps, nummod, :, :][ np.ix_(lat_indices, long_indices)[0], np.ix_(lat_indices, long_indices)[1], 1]
        qr.set_UVC(u,v)
        qr.set_label(str(num))
        ax.set_title(f'Pression : {pres[nummod]} hPA / Altitude : {format(alts[nummod],'.1f')} km')
        
    
    fig,ax = plt.subplots()
    ax.set_xlabel('Longitude')
    ax.set_ylabel('Latitude')
    ax.set_title(f'Pression : {pres[16]} hPA/ Altitude : {format(alts[16],'.1f')} km')
    X,Y,U,V = affiche_vents2D(temps, longmin,longmax,latmin,latmax,0,True,True)
    qr = ax.quiver(X,Y,U,V,color = 'r',pivot = 'middle')


    animm = animation.FuncAnimation(fig, animv, fargs=(qr,temps, longmin, longmax, latmin,latmax),
                              interval=1000, frames=range(17), blit=False)
    if show :
        plt.show()
    if save : 
        animm.save('ventsanim.gif', writer='pillow', fps=1)


## Tests
#affiche_vents3D(0,20,30,-5,5)
#affiche_vents2D(0,10,40,-5,5,200)
affiche_ventsGIF(0,10,40,-5,5,show = True, save = True)